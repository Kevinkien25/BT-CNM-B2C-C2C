...
function HomeContent() {
  ...
  const slides = [
    {
      ...
      description: "Thanh lý đồ cũ nhanh chóng, mua sắm thả ga với cơ chế giữ tiền Escrow an toàn tuyệt đối.",
  ...
  useEffect(() => {
    async function fetchProducts() {
      ...
      try {
        ...
        setError("Chưa kết nối Backend MySQL. Đang hiển thị sản phẩm mẫu hệ thống.");
  ...
  const faqs = [
    { q: "Cơ chế bảo vệ dòng tiền Escrow hoạt động như thế nào?", a: "Khi bạn đặt hàng bằng chuyển khoản, số tiền thanh toán sẽ được RedMall giữ làm trung gian chứ chưa chuyển ngay cho người bán. Chỉ khi bạn nhận được hàng và bấm nút 'Xác nhận đã nhận hàng', sàn mới giải ngân tiền cho người bán. Nếu có khiếu nại, tiền sẽ được giữ lại để xử lý." },
    { q: "Làm thế nào để đăng ký mở gian hàng B2C doanh nghiệp?", a: "Bạn đăng ký tài khoản với vai trò Doanh Nghiệp (B2C Seller), điền thông tin shop kèm theo Mã số thuế. Sau khi Admin hệ thống đối chiếu thông tin và nhấn duyệt, shop của bạn mới chính thức nâng cấp thành Mall và các sản phẩm của bạn sẽ tự động có tag 'Mall' chính hãng màu đỏ." },
    { q: "Tôi là cá nhân, tôi có thể bán đồ cũ trên sàn được không?", a: "Hoàn toàn được! Bạn đăng ký tài khoản với vai trò Cá Nhân (C2C Seller). Cửa hàng cá nhân của bạn sẽ được kích hoạt ngay lập tức mà không cần Admin kiểm duyệt mã số thuế. Bạn có thể đăng bán đồ thanh lý ngay." },
    { q: "Chức năng AI viết mô tả hoạt động như thế nào?", a: "Khi thêm sản phẩm mới trong Kênh Người Bán, bạn chỉ cần gõ Tên sản phẩm, chọn Danh mục và bấm nút 'AI viết mô tả'. Hệ thống sẽ tự động gọi AI Gemini để phân tích và viết một bài mô tả chi tiết chuẩn SEO trong 3 giây." }
  ];
  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden h-[380px] md:h-[450px]">
      ...
      {/* Info notice bar */}
      {error && (
        <div className="bg-amber-50 border-b border-amber-200 text-amber-800 text-xs md:text-sm py-2 px-4 text-center font-medium">
          ⚠️ {error}
        </div>
      )}
      ...
      <section id="categories" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <h2 className="text-xl font-extrabold text-gray-800 tracking-tight mb-6">DANH MỤC SẢN PHẨM</h2>
        <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-4">
          {[
            { name: "Điện thoại & Tablet", icon: "📱", href: "/?search=điện thoại" },
            { name: "Laptop & PC", icon: "💻", href: "/?search=laptop" },
            { name: "Tai nghe & Âm thanh", icon: "🎧", href: "/?search=tai nghe" },
            { name: "Đồ gia dụng", icon: "🔌", href: "/?search=gia dụng" },
            { name: "Thời trang thanh lý", icon: "👕", href: "/?search=áo" },
            { name: "Phụ kiện số", icon: "⌚", href: "/?search=phụ kiện" }
          ].map((cat, idx) => (
            <Link
              key={idx}
              href={cat.href}
              className="bg-white border border-gray-100 hover:border-red-400 hover:shadow-md p-4 rounded-2xl text-center transition-all group"
            >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-12">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 space-y-4">
            ...
            <p className="text-gray-500 text-sm font-medium">Đang tải danh sách sản phẩm...</p>
          </div>
        ) : (
          <>
            {searchQuery && (
              <div className="bg-red-50 text-red-700 py-3 px-6 rounded-2xl flex justify-between items-center">
                <span className="font-medium text-sm">Kết quả tìm kiếm cho: <strong>"{searchQuery}"</strong> ({products.length} sản phẩm)</span>
                <Link href="/" className="text-xs font-bold hover:underline">Xóa bộ lọc</Link>
              </div>
            )}
              ...
              <section className="space-y-6">
                <div className="flex justify-between items-end border-b-2 border-red-600 pb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-xl font-black text-gray-800 tracking-tight">HÀNG CHÍNH HÃNG DOANH NGHIỆP (B2C)</h2>
                  </div>
                  <span className="text-xs text-red-600 font-bold hover:underline cursor-pointer">Xem tất cả &rarr;</span>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
                  {mallProducts.map((prod) => (
                    <div key={prod.id} className="bg-white border border-gray-100 rounded-2xl overflow-hidden hover:shadow-lg transition-all group flex flex-col">
              ...
              <section className="space-y-6">
                <div className="flex justify-between items-end border-b-2 border-gray-800 pb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-xl font-black text-gray-800 tracking-tight">MUA BÁN C2C - THANH LÝ ĐỒ CŨ</h2>
                  </div>
                  <span className="text-xs text-gray-600 font-bold hover:underline cursor-pointer">Xem tất cả &rarr;</span>
                </div>
                    ...
                    <div key={prod.id} className="bg-white border border-gray-100 rounded-2xl overflow-hidden hover:shadow-lg transition-all group flex flex-col">
                      <div className="relative pt-[80%] bg-gray-50 overflow-hidden">
                        <img
                          ...
                          className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                        />
                        <span className="absolute top-2 left-2 bg-gray-700 text-white text-[10px] font-bold uppercase px-2 py-0.5 rounded shadow">Đồ Cũ</span>
                      </div>
                      
                      <div className="p-4 flex-grow flex flex-col justify-between">
                        <div className="space-y-1">
                          <p className="text-xs text-gray-400 font-medium truncate">Chủ shop: {prod.shop_name}</p>
                          <Link href={`/product/${prod.id}`} className="block text-sm font-bold text-gray-800 hover:text-red-600 line-clamp-2 transition-colors h-10">
                            {prod.name}
                          </Link>
                        </div>
                        ...
                        <div className="mt-4 pt-3 border-t border-gray-50 flex items-center justify-between">
                          <div className="space-y-0.5">
                            <p className="text-[10px] text-gray-400">Giá thanh lý</p>
            ...
            {products.length === 0 && (
              <div className="text-center py-16 bg-white border border-gray-100 rounded-3xl space-y-4 shadow-sm">
                ...
                <p className="text-gray-400 text-sm max-w-xs mx-auto">Vui lòng thử tìm kiếm bằng từ khóa khác hoặc quay lại trang chủ.</p>
      ...
      <section className="bg-red-50 py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-black text-center text-gray-900 mb-2">Ý KIẾN KHÁCH HÀNG</h2>
          <p className="text-center text-gray-500 text-sm mb-12">Những nhận xét thực tế từ người bán và người mua trên hệ thống</p>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { name: "Phạm Văn Minh", role: "Người Mua Điện Thoại Cũ", review: "Tôi đã mua chiếc iPhone 11 cũ thanh lý ở đây. Lúc đầu rất lo vì mua hàng cá nhân C2C, nhưng nhờ sàn có cơ chế giữ tiền Escrow nên tôi rất an tâm. Nhận hàng đúng mô tả mới bấm xác nhận.", rating: 5 },
              { name: "Lê Thị Hồng", role: "Doanh Nghiệp Thời Trang", review: "Từ khi được Admin phê duyệt tài khoản B2C lên Mall, doanh số của gian hàng chúng tôi tăng rõ rệt nhờ nhãn hiệu Mall chính hãng nổi bật và tạo dựng lòng tin tối đa với khách hàng.", rating: 5 },
              { name: "Nguyễn Văn Nam", role: "Người Bán C2C Cá Nhân", review: "Chức năng AI viết mô tả sản phẩm của web cực kỳ đỉnh! Tôi chỉ nhập 'Sony WH-1000XM4' mà AI viết ra bài chi tiết đầy đủ thông số, tính năng và cam kết luôn. Đăng bài siêu nhanh.", rating: 5 }
            ].map((rev, idx) => (
      ...
      <section className="bg-white py-12 border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-center text-xs font-bold text-gray-400 tracking-widest uppercase mb-8">ĐỐI TÁC THANH TOÁN VÀ VẬN CHUYỂN TIN CẬY</p>
      ...
      <section className="max-w-4xl mx-auto px-4 py-16 space-y-6">
        <h2 className="text-2xl font-black text-center text-gray-800 mb-2">CÂU HỎI THƯỜNG GẶP</h2>
        <p className="text-center text-gray-500 text-sm mb-8">Giải đáp nhanh các thắc mắc về luồng vận hành của RedMall</p>
        ...
        <div className="space-y-4">
          {faqs.map((faq, idx) => (
            <div key={idx} className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm transition-all">
              ...
              {activeFaq === idx && (
                <div className="px-6 pb-4 pt-1 text-sm text-gray-500 border-t border-gray-50 leading-relaxed">
                  {faq.a}
                </div>
              )}
            </div>
